...
class ZYColumnViewController: UIViewController {
    ...
    fileprivate lazy var spreadView:ZYColumnSpreadView = {
        ...
        spreadView.backgroundColor = UIColor.init(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
    ...
    fileprivate lazy var hScrollView : ZYColumnHScrollView = {
        ...
        hScrollView.frame = CGRect(x: 0, y: 0, width: SCREENW-kTitlesViewH, height: kTitlesViewH)
    ...
    open func hScrollViewChangeItemToSelected(title : String){
        self.hScrollView.changeItemToSelected(title: title);
    }
...
extension ZYColumnViewController {
    fileprivate func setupSpreadButton() {
        ...
        // 包一层UIView 作为背景
        let bgView = UIView(frame: CGRect(x: SCREENW - kTitlesViewH, y: 0.0, width: kTitlesViewH, height: kTitlesViewH))
        ...
        spreadBtn.setImage(UIImage(named: "arrow_index_down_8x4_"), for: UIControlState.normal)
...
//        print("toSpread \(toSpread)  \n self.tabBarController?.tabBar  \(tabbar)")
        tabbar?.isHidden = toSpread           //控制tabBar的隐藏或显示
    }
}
