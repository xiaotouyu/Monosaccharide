...
/************* 上面是对外属性 -----华丽分割线----- 下面是私有属性 ***************/
    ...
    fileprivate lazy var downPromptView : ZYColumnDownPromptView = {
        let count = self.arrayDownItems.count
        let upLastItem = self.arrayUpItems.last
        let downView = ZYColumnDownPromptView(frame: CGRect(x: 0, y: upLastItem!.frame.maxY + kColumnMarginH, width: SCREENW, height: 30))

        downView.backgroundColor = UIColor.init(red: 240.0/255, green: 241.0/255, blue: 246.0/255, alpha: 1.0)

        if self.arrayDownItems.isEmpty {
...
//            downPromptView.isDownPromptView = true
            downPromptView.isHiddenMore = true

        } else {
            downPromptView.isHidden = toHide
//            downPromptView.isDownPromptView = toHide
            downPromptView.isHiddenMore = toHide
...
extension ZYColumnSpreadView {
    fileprivate func addItems(){
        ...
        for i in 0 ..< arrayDownTitles.count {
            downPromptView.isHiddenMore = false
